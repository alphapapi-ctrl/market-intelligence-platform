...
def calculate_benchmark(prices, prices_24m, volumes, watchlist_df):
    ...
    # Build benchmark price series per commodity
    bench_rows   = watchlist_df[watchlist_df['benchmark'] == 'benchmark']
    bench_prices = {}
    for _, row in bench_rows.iterrows():
        ticker = row['ticker']
        comm   = row['commodity']
        if ticker in prices.columns:
            bp = prices[ticker].dropna()
            if len(bp) >= 2:
                bench_prices[comm] = bp
                print(f"  Benchmark for {comm}: {ticker}")
    ...
    for ticker in tickers:
        ...
        comm = commodity.get(ticker, 'unknown')

        # Get benchmark for this commodity
        bench = bench_prices.get(comm)
        if bench is None:
            continue

        bench_12m = (bench.iloc[-1] / bench.iloc[0]) - 1
        bench_5d  = (bench.iloc[-1] / bench.iloc[-5]  - 1) if len(bench) >= 5  else 0
        bench_21d = (bench.iloc[-1] / bench.iloc[-21] - 1) if len(bench) >= 21 else 0
        bench_63d = (bench.iloc[-1] / bench.iloc[-63] - 1) if len(bench) >= 63 else 0

        # RS Ratio vs commodity benchmark
...
def save_results(df, results_dir, study_name):
    ...
    # Save per commodity
    comm_dir = f"results/benchmark/commodities/"
    os.makedirs(comm_dir, exist_ok=True)
    for comm in df['commodity'].unique():
        comm_df   = df[df['commodity'] == comm].copy()
        comm_file = f"{comm_dir}{today}_{comm}.csv"
        comm_df.to_csv(comm_file)

    # Save per type within commodity
    for comm in df['commodity'].unique():
        for typ in df['type'].unique():
            sub_df = df[(df['commodity'] == comm) & (df['type'] == typ)].copy()
            if len(sub_df) > 0:
                sub_file = f"{comm_dir}{today}_{comm}_{typ}.csv"
    ...
    actionable_df.to_csv(f"{actionable_dir}{today}_commodities_actionable.csv")

    tv_tickers = ','.join(actionable_df['ticker'].tolist())
    with open(f"{actionable_dir}{today}_commodities_actionable_tvimport.txt", 'w') as f:
        f.write(tv_tickers)
...
if __name__ == "__main__":
    ...
    if bm_results is not None:
        ...
        print(f"No results generated for {STUDY_NAME} — check price data and try again")