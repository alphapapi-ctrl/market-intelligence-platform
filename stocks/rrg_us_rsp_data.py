...
OUTPUT_FILE   = 'results/rrg/us_rrg_rsp_history.csv'
BENCHMARK     = 'RSP'  # Equal-weight S&P 500
...
if __name__ == "__main__":
    ...
    # Check existing history to determine what needs fetching
    if os.path.exists(OUTPUT_FILE):
        history          = pd.read_csv(OUTPUT_FILE)
        existing_tickers = set(history['ticker'].unique())
        new_tickers      = [t for t in TICKERS.keys() if t not in existing_tickers]
        ...
        if new_tickers:
            # Re-fetch with full BACKFILL_DAYS range so new tickers get complete history
            print(f"New tickers detected — fetching full history: {new_tickers}")
