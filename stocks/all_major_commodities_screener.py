...
_RS = _load_rs('comm_screener')
from config.screener.study_all_major_commodities import config
from data_fetch.screener.data_fetch_all_major_commodities import load_watchlist, fetch_prices, fetch_volumes
...
for _p in [
    ...
    if os.path.exists(_p):
        try: _RS = _json.load(open(_p)).get('comm_screener', {}); break
        except: pass
...
def calculate_screener(prices, prices_24m, volumes, watchlist_df):
    ...
    tickers = [t for t in prices.columns
               ...
               and commodity.get(t) is not None
               and len(prices[t].dropna()) >= 2]

    # Pre-calculate 12M returns for commodity peer RS
    returns_12m = {}
    ...
    for ticker in tickers:
        ...
        if comm not in commodity_tickers:
            commodity_tickers[comm] = []
        commodity_tickers[comm].append(ticker)
    ...
    for ticker in tickers:
        ...
        # Commodity peer RS score
        comm_peers = [t for t in commodity_tickers.get(comm, [])
                      if t != ticker and t in returns_12m]
        if len(comm_peers) > 0:
            outperforms   = sum(1 for t in comm_peers if ret_12m > returns_12m[t])
            peer_rs_score = round(outperforms / len(comm_peers) * 100, 2)
        else:
            peer_rs_score = 50.0

        # Historical peer RS vs commodity median
        def comm_median(n):
            vals = [prices[t].dropna().iloc[-1] / prices[t].dropna().iloc[-n] - 1
                    for t in comm_peers if len(prices[t].dropna()) >= n]
            return np.median(vals) if vals else 0

        rs_5  = round((ticker_prices.iloc[-1] / ticker_prices.iloc[-5]  - 1) - comm_median(5),  4) if len(ticker_prices) >= 5  else None
        rs_21 = round((ticker_prices.iloc[-1] / ticker_prices.iloc[-21] - 1) - comm_median(21), 4) if len(ticker_prices) >= 21 else None
        rs_63 = round((ticker_prices.iloc[-1] / ticker_prices.iloc[-63] - 1) - comm_median(63), 4) if len(ticker_prices) >= 63 else None
        ...
        regime_bonus = {'LEADER': _RS.get('regime_bonus_leader', 1.0), 'CONTENDER': _RS.get('regime_bonus_contender', 0.5), 'LAGGARD': _RS.get('regime_bonus_laggard', 0.0), 'WEAK': _RS.get('regime_bonus_weak', -0.5)}
        base_score  = (
            ...
            (peer_rs_score * _RS.get('peer_rs_weight', 0.02)) +
...
def save_results(df, results_dir, study_name):
    ...
    # Reorder columns
    df = df[['delta_rank', 'ticker', 'name', 'commodity', 'type', 'cap_band', 'close',
    ...
    # Save per commodity
    comm_dir = f"results/screener/commodities/"
    os.makedirs(comm_dir, exist_ok=True)
    for comm in df['commodity'].unique():
        comm_df = df[df['commodity'] == comm].copy()
        comm_df.to_csv(f"{comm_dir}{today}_{comm}.csv")

    # Save per commodity per type
    for comm in df['commodity'].unique():
        for typ in df['type'].unique():
            sub_df = df[(df['commodity'] == comm) & (df['type'] == typ)].copy()
            if len(sub_df) > 0:
                type_key = str(typ).lower().replace(' ', '_').replace('/', '_')
                sub_df.to_csv(f"{comm_dir}{today}_{comm}_{type_key}.csv")
    ...
    actionable_df = df[
        ...
        ((df['vol_label'] == 'HIGH') & (df['regime_label'].isin(['LEADER', 'CONTENDER'])))
    ].copy()
    actionable_df.to_csv(f"{actionable_dir}{today}_commodities_screener_actionable.csv")

    tv_tickers = ','.join(actionable_df['ticker'].tolist())
    with open(f"{actionable_dir}{today}_commodities_screener_actionable_tvimport.txt", 'w') as f:
        f.write(tv_tickers)
