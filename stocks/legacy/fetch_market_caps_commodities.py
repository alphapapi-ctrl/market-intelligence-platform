...
WATCHLIST_FILE = os.path.join(BASE, 'stocks', 'watchlist', 'all_major_commodities.csv')
OUTPUT_FILE    = WATCHLIST_FILE

# ── Cap band thresholds — commodities ────────────────────────────────────────
def get_cap_band(market_cap, ticker_type=None):
    if ticker_type == 'ETF':
        return 'ETF'
...
if __name__ == "__main__":
    ...
    df['cap_band']   = df.apply(
        lambda row: get_cap_band(row['market_cap'], row.get('type')), axis=1
    )
    ...
    print(f"\nBy commodity:")
    print(df.groupby(['commodity','cap_band']).size().unstack(fill_value=0))