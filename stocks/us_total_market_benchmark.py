...
_RS = _load_rs('us_benchmark')
from config.benchmark.study_us_total_market import config
...
def calculate_benchmark(prices, prices_24m, volumes, watchlist_df):
    ...
    if len(bench_row) == 0:
        print("ERROR: No benchmark row found in watchlist")
        return None
    ...
    if len(bench_prices) < 2:
        print("ERROR: Insufficient benchmark price data")
        return None
    ...
    for ticker in tickers:
        ...
        # RS Ratio vs benchmark — return based
        ...
        # Regime Label — TREND+LEAD when rs_ratio > 1.0
...
def save_results(df, results_dir, study_name):
    ...
    # Save per sector
    sector_dir = 'results/benchmark/us_sectors/'
    ...
    actionable_df.to_csv(f"{actionable_dir}{today}_us_benchmark_actionable.csv")

    tv_tickers = ','.join(actionable_df['ticker'].tolist())
    with open(f"{actionable_dir}{today}_us_benchmark_actionable_tvimport.txt", 'w') as f:
        f.write(tv_tickers)
