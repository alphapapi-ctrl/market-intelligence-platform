...
from config.screener.study_nasdaq100 import config
from data_fetch.screener.data_fetch_us_total_market import load_watchlist, fetch_prices, fetch_volumes
...
for _p in [
    ...
    if os.path.exists(_p):
        try: _RS = _json.load(open(_p)).get('us_screener', {}); break
        except: pass
...
def calculate_screener(prices, prices_24m, volumes, watchlist_df):
    ...
    # Resolve cap_band — handle empty/NaN values in watchlist
    ...
    tickers  = [t for t in prices.columns
                if t not in (watchlist_df[watchlist_df['sector'] == 'index']['ticker'].tolist())
    ...
    for ticker in tickers:
        ...
        # Volatility
        vol_63 = round(ticker_prices.pct_change().dropna().tail(63).std() * (252 ** 0.5) * 100, 2) if len(ticker_prices) >= 63 \
                 else round(ticker_prices.pct_change().dropna().std() * (252 ** 0.5) * 100, 2)
        ...
        regime_bonus = {
            'LEADER'    : _RS.get('regime_bonus_leader',     1.0),
            'CONTENDER' : _RS.get('regime_bonus_contender',  0.5),
            'LAGGARD'   : _RS.get('regime_bonus_laggard',    0.0),
            'WEAK'      : _RS.get('regime_bonus_weak',      -0.5),
        }
        base_score  = (
...
def save_results(df, results_dir, study_name):
    ...
    # Per-sector files
    sector_dir = 'results/screener/nasdaq100_sectors/'
    ...
    tv_tickers = ','.join([f"NASDAQ:{t}" for t in actionable_df['ticker'].tolist()])
    ...
    tv_highconv = ','.join([f"NASDAQ:{t}" for t in highconv_df['ticker'].tolist()])
