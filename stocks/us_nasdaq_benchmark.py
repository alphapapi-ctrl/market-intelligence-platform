...
from config.benchmark.study_us_nasdaq_benchmark import config
from data_fetch.benchmark.data_fetch_us_total_market import load_watchlist, fetch_prices, fetch_volumes
...
for _p in [
    ...
    if os.path.exists(_p):
        try: _RS = _json.load(open(_p)).get('us_benchmark', {}); break
        except: pass
...
BENCHMARK_TICKER = config['benchmark_ticker']   # '^NDX'
END_DATE         = datetime.today().strftime('%Y-%m-%d')
START_DATE       = (datetime.today() - timedelta(days=365)).strftime('%Y-%m-%d')
START_24M        = (datetime.today() - timedelta(days=730)).strftime('%Y-%m-%d')

# ── Inject benchmark prices into price DataFrame ────────────────────────────────
def inject_benchmark(prices, bench_ticker, start, end):
    """Fetch the benchmark ticker and add it to the prices DataFrame."""
    if bench_ticker in prices.columns:
        return prices
    print(f"Fetching benchmark {bench_ticker}...")
    raw = yf.download(bench_ticker, start=start, end=end, auto_adjust=True, progress=False)
    if raw.empty:
        print(f"ERROR: Could not fetch {bench_ticker}")
        return prices
    bench_series = raw['Close']
    if hasattr(bench_series, 'squeeze'):
        bench_series = bench_series.squeeze()
    prices = prices.copy()
    prices[bench_ticker] = bench_series
    return prices

# ── Calculate Benchmark RS vs ^NDX ─────────────────────────────────────────────
def calculate_benchmark(prices, prices_24m, volumes, watchlist_df, bench_ticker):
    results   = []
    names     = watchlist_df.set_index('ticker')['name']
    sectors   = watchlist_df.set_index('ticker')['sector'].to_dict()
    industry  = watchlist_df.set_index('ticker')['industry'].to_dict() if 'industry' in watchlist_df.columns else {}

    # Resolve cap_band — handle empty/NaN values
    raw_cap   = watchlist_df.set_index('ticker')['cap_band'].to_dict() if 'cap_band' in watchlist_df.columns else {}
    cap_bands = {k: (v if isinstance(v, str) and v else 'small') for k, v in raw_cap.items()}
    ...
    if len(bench_prices) < 2:
        print(f"ERROR: Insufficient benchmark data for {bench_ticker}")
        return None

    print(f"Benchmark: {bench_ticker} ({len(bench_prices)} trading days)")
    ...
    for ticker in tickers:
        ...
        if len(ticker_prices) < 2 or ticker_prices.iloc[0] == 0:
            continue
        ...
        # RS Ratio vs benchmark
        rs_ratio = round((1 + ret_12m) / (1 + bench_12m), 4)

        rs_5  = round((1 + (ticker_prices.iloc[-1] / ticker_prices.iloc[-5]  - 1)) /
                      (1 + bench_5d),  4) if len(ticker_prices) >= 5  else None
        rs_21 = round((1 + (ticker_prices.iloc[-1] / ticker_prices.iloc[-21] - 1)) /
                      (1 + bench_21d), 4) if len(ticker_prices) >= 21 else None
        rs_63 = round((1 + (ticker_prices.iloc[-1] / ticker_prices.iloc[-63] - 1)) /
                      (1 + bench_63d), 4) if len(ticker_prices) >= 63 else None
        ...
        ticker_24m  = prices_24m[ticker].dropna() if ticker in prices_24m.columns else pd.Series(dtype=float)
        ...
        # Regime Label — TREND+LEAD when outperforming ^NDX
...
def save_results(df, results_dir, study_name):
    ...
    sector_dir = 'results/benchmark/us_nasdaq_sectors/'
    os.makedirs(sector_dir, exist_ok=True)
    for sector in df['sector'].unique():
        sec_df   = df[df['sector'] == sector].copy()
        sec_name = str(sector).lower().replace(' ', '_').replace('/', '_')
        sec_df.to_csv(f"{sector_dir}{today}_{sec_name}.csv")
    ...
    actionable_df.to_csv(f"{actionable_dir}{today}_{study_name}_actionable.csv")
    tv_tickers = ','.join(actionable_df['ticker'].tolist())
    with open(f"{actionable_dir}{today}_{study_name}_actionable_tvimport.txt", 'w') as f:
        f.write(tv_tickers)
    print(f"Actionable saved to {actionable_dir}")
...
if __name__ == "__main__":
    ...
    # Inject ^NDX into price data (it's not in us_all_watchlist)
    prices     = inject_benchmark(prices,     BENCHMARK_TICKER, START_DATE, END_DATE)
    prices_24m = inject_benchmark(prices_24m, BENCHMARK_TICKER, START_24M,  END_DATE)

    bm_results = calculate_benchmark(prices, prices_24m, volumes, watchlist, BENCHMARK_TICKER)
