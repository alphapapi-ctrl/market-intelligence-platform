...
def main():
    ...
    for set_path in set_files:
        ...
        # EA_Comment
        ea_comment = f"{name_stem} {symbol} {period}"
        ...
        for search_dir in search_dirs:
            htm_src = os.path.join(search_dir, name_stem + '.htm')
            if os.path.isfile(htm_src):
                htm_dest = os.path.join(file_report_dir, name_stem + '.htm')
                shutil.copy2(htm_src, htm_dest)
                copied = [name_stem + '.htm']
                for fn in os.listdir(search_dir):
                    if fn.startswith(name_stem) and not fn.endswith('.htm'):
