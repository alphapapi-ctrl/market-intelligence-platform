...
from config.screener.study_uranium import config
from data_fetch.screener.data_fetch_uranium import load_watchlist, fetch_prices, fetch_volumes
...
def save_results(df, results_dir, study_name):
    ...
    highconv_df.to_csv(f"{actionable_dir}{today}_uranium_screener_highconv.csv")

    tv_highconv = ','.join(highconv_df['ticker'].tolist())
    with open(f"{actionable_dir}{today}_uranium_screener_highconv_tvimport.txt", 'w') as f:
        f.write(tv_highconv)
