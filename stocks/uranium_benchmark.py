...
from config.benchmark.study_uranium import config
from data_fetch.benchmark.data_fetch_uranium import load_watchlist, fetch_prices, fetch_volumes
...
def save_results(df, results_dir, study_name):
    ...
    actionable_df.to_csv(f"{actionable_dir}{today}_uranium_actionable.csv")

    tv_tickers = ','.join(actionable_df['ticker'].tolist())
    with open(f"{actionable_dir}{today}_uranium_actionable_tvimport.txt", 'w') as f:
        f.write(tv_tickers)
