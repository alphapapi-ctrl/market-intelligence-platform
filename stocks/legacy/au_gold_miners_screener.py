...
from config.screener.study_au_gold_miners import config
from data_fetch.screener.data_fetch_au_gold_miners import load_watchlist, fetch_prices, fetch_volumes
...
def calculate_screener(prices, prices_24m, volumes, watchlist_df):
    ...
    tickers = [t for t in prices.columns
               if sectors.get(t) != 'index'
               and len(prices[t].dropna()) >= 2]

    # Pre-calculate 12M returns for peer RS
    returns_12m = {}
...
def save_results(df, results_dir, study_name):
    ...
    actionable_df.to_csv(f"{actionable_dir}{today}_uranium_screener_actionable.csv")

    tv_tickers = ','.join(actionable_df['ticker'].tolist())
    with open(f"{actionable_dir}{today}_uranium_screener_actionable_tvimport.txt", 'w') as f:
        f.write(tv_tickers)
    ...
    highconv_df.to_csv(f"{actionable_dir}{today}_au_gold_miners_screener_highconv.csv")

    tv_highconv = ','.join(highconv_df['ticker'].tolist())
    with open(f"{actionable_dir}{today}_au_gold_miners_screener_highconv_tvimport.txt", 'w') as f:
        f.write(tv_highconv)
