...
WATCHLIST_FILE = os.path.join(BASE, 'stocks', 'watchlist', 'nasdaq100.csv')
OUTPUT_FILE    = WATCHLIST_FILE

# ── Cap band thresholds — US market ──────────────────────────────────────────
...
def fetch_market_caps(watchlist_df, batch_size=50, delay=1):
    # Skip index/benchmark rows
    skip    = set(watchlist_df[watchlist_df['sector'] == 'index']['ticker'].tolist())
    tickers = [t for t in watchlist_df['ticker'].tolist() if t not in skip]
...
if __name__ == "__main__":
    ...
    print(f"\nBy sector:")
    print(df.groupby(['sector', 'cap_band']).size().unstack(fill_value=0))
