...
# ── Identify benchmark ────────────────────────────────────────────────────────
def get_benchmark(watchlist_df):
    bench = watchlist_df[watchlist_df['benchmark'] == 'benchmark']['ticker'].values[0]
    print(f"Benchmark: {bench}")
    return bench

# ── Cap band ──────────────────────────────────────────────────────────────────
def get_cap_band(row):
    if row['benchmark'] == 'benchmark':
        return 'ETF'
    return row.get('cap_band', 'small')

# ── Calculate benchmark RS ────────────────────────────────────────────────────
def calculate_rs(prices, prices_24m, volumes, benchmark_ticker, watchlist_df):
    results  = []
    ...
    bench_12m_return = (benchmark_prices.iloc[-1] / benchmark_prices.iloc[0]) - 1
    ...
    for ticker in prices.columns:
        ...
        # RS Ratio
        rs_ratio = round((1 + ret_12m) / (1 + bench_12m_return), 4)

        # Historical RS ratios
        bench_ret_5  = (benchmark_prices.iloc[-1] / benchmark_prices.iloc[-5]  - 1) if len(benchmark_prices) >= 5  else None
        bench_ret_21 = (benchmark_prices.iloc[-1] / benchmark_prices.iloc[-21] - 1) if len(benchmark_prices) >= 21 else None
        bench_ret_63 = (benchmark_prices.iloc[-1] / benchmark_prices.iloc[-63] - 1) if len(benchmark_prices) >= 63 else None

        tick_ret_5   = (ticker_prices.iloc[-1] / ticker_prices.iloc[-5]  - 1) if len(ticker_prices) >= 5  else None
        tick_ret_21  = (ticker_prices.iloc[-1] / ticker_prices.iloc[-21] - 1) if len(ticker_prices) >= 21 else None
        tick_ret_63  = (ticker_prices.iloc[-1] / ticker_prices.iloc[-63] - 1) if len(ticker_prices) >= 63 else None

        rs_5  = round((1 + tick_ret_5)  / (1 + bench_ret_5),  4) if tick_ret_5  is not None and bench_ret_5  is not None else None
        rs_21 = round((1 + tick_ret_21) / (1 + bench_ret_21), 4) if tick_ret_21 is not None and bench_ret_21 is not None else None
        rs_63 = round((1 + tick_ret_63) / (1 + bench_ret_63), 4) if tick_ret_63 is not None and bench_ret_63 is not None else None

        # RS Trend
        rs_values = [r for r in [rs_63, rs_21, rs_5, rs_ratio] if r is not None]
        if len(rs_values) >= 2:
        ...
        # Regime Label
        if pass_trend == 1 and rs_ratio > 1.0:
    ...
    if df.empty or 'score_final' not in df.columns:
        print(f"Warning: no results calculated for {STUDY_NAME} — price data may be unavailable")
        return None
...
def save_results(df, results_dir, study_name):
    ...
    # Save per sector
    sector_dir = f"results/benchmark/au_sectors/"
    ...
    # Daily actionable
    actionable_dir = f"results/daily_actionable/benchmark/"
...
if __name__ == "__main__":
    ...
    benchmark   = get_benchmark(watchlist)
    rs_results  = calculate_rs(prices, prices_24m, volumes, benchmark, watchlist)
